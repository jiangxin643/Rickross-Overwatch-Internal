...
Menu menu_t_instance = {};
Menu* g_tMenu = &menu_t_instance;
...
void Menu::Init()
{
	...
	this->menuInfo.y		= 350;	// posição y do menu
	this->menuInfo.w		= 450;	// largura do menu
...
void Menu::AddTab(char* name)
{
//	this->tabs[this->tabInfo.num].name = name;
	memcpy(this->tabs[this->tabInfo.num].name, name, strlen(name));
	this->tabs[this->tabInfo.num].pos = itemInfo.num;
	this->tabInfo.num++;
}

void Menu::AddItem(char* name, char** display, int max, int value)
{
//	this->items[this->itemInfo.num].name = name;
	memcpy(this->items[this->itemInfo.num].name, name, strlen(name));
...
//	draw.Rectangle(0, 0, 1920 / 2.0f, 1080 / 2.0f, StartColor, EndColor, BorderColor);
	
	// cabeçalho
	draw.Rectangle((float)(x), (float)(y), w, 20, StartColor, EndColor, BorderColor);
...
{
	this->c.isLClicked = g_menu->_mouse1_click;
	this->c.isRClicked = false;
	this->c.isHolding  = g_menu->_mouse1_state;
}
...
void Menu::DrawTabs(int x, int y, int w, int h)
{
	for (int i = 0; i < this->tabInfo.num; i++)
	{
		...
		draw.Text(x + w/3, y + (i * h) + (h/2) - 8, this->tabs[i].name, color, false, TextAlignment::kCenter);
	}
}
...
void Menu::DrawItems(int x, int y, int w)
{
	...
	for (int i = this->FirstScrollIndex(); i <= this->LastScrollIndex(); i++)
	{
		...
		// nome da função
		draw.Text(x + 10, itemOffset + 5, this->items[i].name, color);

		// lista de opções
		draw.Text(x + w - w/4 - 60, itemOffset + 5, this->items[i].display[this->items[i].value], color, false, TextAlignment::kRight);
